...
from app.workflow import GroqResponder
...
async def test_generates_mapped_citations_from_selected_context_only() -> None:
    ...
    result = await GroqResponder(Settings(), gateway).answer("What is the UPS autonomy?", context())
...
async def test_rejects_invented_unit_for_real_numeric_value() -> None:
    ...
    result = await GroqResponder(Settings(), FakeGateway(response)).answer("What is the UPS autonomy?", context())
...
async def test_rejects_invalid_citation_id() -> None:
    with pytest.raises(IngestionError, match="unknown citation") as error:
        await GroqResponder(Settings(), FakeGateway(generated(citation_id="C9"))).answer(
...
async def test_returns_insufficient_without_calling_model_for_empty_context() -> None:
    ...
    result = await GroqResponder(Settings(), gateway).answer("Unknown value?", bundle)
...
async def test_reports_context_revision_conflict() -> None:
    ...
    result = await GroqResponder(Settings(), FakeGateway(generated())).answer(bundle.query, bundle)
...
async def test_uses_one_semantic_call_only_for_uncertain_claim() -> None:
    ...
    result = await GroqResponder(Settings(), gateway).answer("Is battery performance adequate?", context())
