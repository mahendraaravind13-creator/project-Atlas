...
**Measured:** 5/5 shipments represented, 15 supplier tiers, mean alert latency **55 minutes**, alternatives generated for all risky shipments.
...
Tabs: Project overview · Documents & ingestion · Knowledge / RFI · Compliance findings · Equipment thread · Impact Chain · Commissioning readiness · Supply-chain risk · Evidence Dashboard · Evaluation dashboard · Mitigation simulator.

**Two UI decisions worth pointing at:**
...
A labelled synthetic corpus with **planted** defects and a `ground_truth.json` — 3 specifications, 6 submittals (3 clean, 3 carrying six planted deviations), 12 RFIs (with two planted near-duplicate pairs), a schedule CSV with one planted 35-day delay, 3 commissioning templates, 5 shipments.
...
- **Authentication, RBAC, tenant quotas, signed URLs.** `project_id` filtering is application-level isolation, *not* an authorisation system. The demo must not be exposed publicly.
...
- **Production object storage** — originals sit on the local filesystem.
...
- Commissioning: **21/21** steps, coverage 1.0, NCR 1/1
- **135 backend tests, 9 frontend tests**
...
> ⚠️ **`FINAL_STATUS.md` is stale** — it reports 88 backend tests. The suite is now **135**. Update it before submission.
